...
private:
    ...
    QColor inColorStart;            //导航条内层阴影渐变开始颜色
    QColor inColorEnd;              //导航条内层阴影渐变结束颜色

    QColor barColorStart;           //导航条当前条目渐变开始颜色
    QColor barColorEnd;             //导航条当前条目渐变结束颜色
    QColor HoverColorStart;         //导航条悬停条目渐变开始颜色
    QColor HoverColorEnd;           //导航条悬停条目渐变结束颜色
    ...
    int currentHoverIndex;          //当前悬停的文字索引

    int bgRadius;                   //背景圆角半径
    int barRadius;                  //选中条目背景圆角半径
    int HoverRadius;                //悬停条目背景圆角半径
    int space;                      //条目元素之间的间距
    ...
    bool flat;                      //是否扁平化
    bool isHovered;                 //追踪鼠标是否悬停

    //元素集合,成对出现,元素的名字,矩形区域范围
    QVector<QPair<QString, QRectF> > listItem;

    QRectF barRect, targetRect, HoverRect;     //选中区域和目标区域的矩形
...
Q_SIGNALS:
    ...
    void signalItemCutPage(int index);
};
